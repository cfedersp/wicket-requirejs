...
public final class RequireJs {
     ...
     */
    public static void install(final WebApplication application, IRequireJsSettingsFactory settingsFactory) {
        if (application.getMetaData(REQUIRE_JS_SETTINGS_META_DATA_KEY) == null) {
            WicketWebjars.install(application);

            if (settingsFactory == null) {
                settingsFactory = new DefaultRequireJsSettingsFactory();
            }
		RequireJsSettings settings = settingsFactory.create();
